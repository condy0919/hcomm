...
template <typename F, typename C = std::invoke_result_t<F>>
    requires Continuation<C>
class RepeatContinuation {
public:
    explicit RepeatContinuation(F f) : handle_(std::move(f)) {}

    auto operator()(Context& ctx) -> Result<void, typename C::ErrorType> {
        while (true) {
            if (!cont_) {
                cont_.emplace(handle_());
            }
            ...
            if (!result) {
                ...
                if constexpr (std::is_void_v<typename C::ErrorType>) {
                    return Err();
                } else {
                    return Err(result.takeError());
                }
            }

            // Reset the current promise after it completes.
            // Promises are one-shot use objects that consume their continuation upon completion; re-invoking a
            // completed promise is undefined behavior.
            cont_.reset();
        }
    }

private:
    MoveOnlyHandler<F> handle_;
...
/// Creates a promise that repeatedly executes promises generated by a factory function.
///
/// This is a high-level combinator for implementing long-running asynchronous loops. Once the
/// current promise completes, the factory is re-invoked to produce the next promise in the
/// sequence. This process continues indefinitely as long as the promises complete.
///
/// Example:
/// ```cpp
/// // A simple periodic task that logs a heartbeat every 5 seconds
/// exec->schedule(repeat([] {
///     return sleepFor(std::chrono::seconds(5)).andThen([]() -> Result<> {
///         HCOMM_LOG_INFO("Heartbeat...");
///         return Ok();
///     });
/// }));
/// ```
template <typename F>
auto repeat(F f) {
    return PromiseImpl(internal::RepeatContinuation<F>(std::move(f)));
}
